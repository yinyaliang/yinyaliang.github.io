var store = [{
        ...
        "title": "ospf笔记-特殊区域",
        "excerpt":"OSPF备忘信息 标准区域 这是默认的区域类型，它接受链路更新、汇总路由和外部路由。 骨干区域 骨干区域为区域0，其他区域都与之相连以交换和路由信息。OSPF骨干区域具有标准OSPF区域的所有特征 末节区域 这种区域不接受关于自治系统外部的路由的信息，如来自非 OSPF 路由器的路由。需要路由到自治系统外部的网络时，路由器使用默认路由（用0.0.0.0表示）。末节区域不能包含ASBR（除非ABR也是ASBR） 绝对末节区域 这种 Cisco 专用的区域不接受来自自治系统外部的路由以及来自自治系统中其他区域的汇总路由。需要将分组发送到区域外的网络时，路由器使用默认路由。绝对末节区域中不能有ASBR（除非ABR也是ASBR） NSSA NSSA 是对OSPF RFC的补充。这种区域定义了一种特殊的 LSA——7类LSA。NSSA具有末节区域的优点，它们不接受有关自治系统外部的路由的信息，而使用默认路由前往外部网络。然而，NSSA可以包含ASBR，这违反了关于末节区域的规则。 绝对末节NSSA Cisco路由器也允许将区域配置为绝对末节NSSA，这种区域可包含 ASBR，但不接受外部路由和来自其他区域的汇总路由。它使用默认路由前往区域外的网络。 配置末节区域 步骤 在区域内的所有路由器上配置OSPF 在区域中所有的路由器上配置路由器配置命令area area-id stub （可选）在ABR中配置默认路由的开销 配置绝对末节区域(CISCO专有) 5类外部LSA以及3类和4类汇总LSA（区域间路由）不能传播到绝对末节区域中 步骤 在区域内的所有路由器上配置OSPF 在区域中所有的路由器上配置路由器配置命令area area-id stub 在ABR上，在命令area area-id stub中指定关键字no-summary，这让区域变成绝对末节的 #（可选）在ABR上，配置默认路由的开销 OSPF的路由表 （没有任何末节配置的）标准区域中OSPF路由器的路由表，其中包括区域内路由（O）、区域间路由（O IA）和外部路由（O E1和 O E2） 配置为末节区域后的路由表，其中包含区域内路由（O）和区域间路由（O IA），但不包含外部路由（O E1和 O E2），因为这些路由被区域间默认路由（O*IA）取代...","categories": [],
        "tags": ["RS"],
        "url": "/rs-ospf-05/",
        "teaser": null
      }]
